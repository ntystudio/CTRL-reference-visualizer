...
struct CTRLREFERENCEVISUALIZER_API HCrvHitProxy : public HComponentVisProxy
{
	...
	HCrvHitProxy(const UActorComponent* ParentComponent, const AActor* InTargetActor)
		: HComponentVisProxy(ParentComponent, HPP_Foreground),
		  TargetObject(InTargetActor)
	{
	}

	HCrvHitProxy(const UActorComponent* ParentComponent, const UActorComponent* InTargetComponent)
		: HComponentVisProxy(ParentComponent, HPP_Foreground),
